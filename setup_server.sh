...
AWS_SECRET_ACCESS_KEY=$(cat aws_creds/backup-postgres-appendonly.accesskey.json | python -c 'import json,sys; obj=json.load(sys.stdin); print obj["AccessKey"]["SecretAccessKey"]')
...
echo s3://backup-postgres-danstutzman | sudo tee /etc/wal-e.d/env/WALE_S3_PREFIX
